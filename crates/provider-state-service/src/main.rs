...
fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let appearance_capability = appearance_capability_file
        .map(
            |path| -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
                let metadata = fs::metadata(&path)?;
                if !metadata.is_file() || metadata.len() == 0 || metadata.len() > 257 {
                    return Err("appearance capability file must contain 1 to 256 bytes".into());
                }
                if metadata.permissions().mode() & 0o077 != 0 {
                    return Err(
                        "appearance capability file must not be group/world accessible".into(),
                    );
                }
        ...
        .transpose()?;
    provider_state_service::serve_with_appearance_writer(
        &socket,
        &state_file,
