...
long official image run. That is not an in-repo generator we can keep honest,
so this path remixes an official Fedora Workstation live ISO by baking the
same offline selectable-session install the hardware gate already uses.

The remix is live-boot evidence media. It is not an installed product, not a
stock Fedora live USB, and not the boot-owned appliance target.
...
- `/usr/share/doc/sos/image-identity.env` plus ISO-level
  `/sos-image-identity.env` with live-boot labels and payload hashes
...
Download a current official Fedora Workstation live x86_64 ISO from Fedora.
Keep that file and its SHA-256. Then, from a clean checkout of the revision
...
Bake requires sudo for destroot mutation and `dnf --installroot`. It preserves
the source ISO volume ID and El Torito/EFI boot; changing the label breaks
Fedora's `root=live:CDLABEL=...` cmdline. The output sidecar
`artifacts/linux-live-image/image-identity.env` records the source revision,
base ISO identity, live squashfs or erofs payload hash, and output ISO hash.
...
Prepare and collect record `boot_kind=live-boot` and
`not_installed_product=true`. Hardware (DRM, input, DMI) is the same silicon
as an installed Fedora Workstation. Persistence, disk, and bootloader are
not. Do not call this campaign an installed product.
