...
/// Raw mobile pointer data before platform compatibility code maps it to a
/// single mouse/scroll stream. Permanent hosts can opt into this bounded
/// observation hook without taking ownership of the platform window callback.
...
type RawTouchCallback = Box<dyn FnMut(RawTouchEvent) + Send>;
static RAW_TOUCH_CALLBACK: parking_lot::Mutex<Option<RawTouchCallback>> =
    parking_lot::Mutex::new(None);

pub fn set_raw_touch_callback(callback: Option<RawTouchCallback>) {
    *RAW_TOUCH_CALLBACK.lock() = callback;
}

#[cfg_attr(not(target_os = "android"), allow(dead_code))]
pub(crate) fn dispatch_raw_touch(event: RawTouchEvent) {
    if let Some(callback) = RAW_TOUCH_CALLBACK.lock().as_mut() {
        callback(event);
    }
}
