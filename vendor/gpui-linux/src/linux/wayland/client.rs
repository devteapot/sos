...
use crate::{RawTouchEvent, RawTouchPhase, dispatch_raw_touch};
use gpui::{
...
struct RawTabletTool {
    ...
    down: bool,
    pending_motion: bool,
    pending_phase: Option<RawTouchPhase>,
    window: Option<WaylandWindowStatePtr>,
}

impl Default for RawTabletTool {
    fn default() -> Self {
        Self {
            x: 0.0,
            y: 0.0,
...
pub(crate) struct WaylandClientState {
    ...
    touch_contacts: HashMap<i32, RawTouchContact>,
    tablet_seat: Option<zwp_tablet_seat_v2::ZwpTabletSeatV2>,
    tablet_tools: HashMap<ObjectId, RawTabletTool>,
...
impl Dispatch<wl_seat::WlSeat, ()> for WaylandClientStatePtr {
    fn event(
        ...
        } = event
        {
            let client = state.get_client();
            let mut state = client.borrow_mut();
            if state.tablet_seat.is_none() {
                state.tablet_seat = state
                    .globals
                    .tablet_manager
                    .as_ref()
        ...
        let dispatched = {
            ...
            match event {
                wl_touch::Event::Down {
                    ...
                    let contact = RawTouchContact {
                        x: x as f32,
                        y: y as f32,
                        time_millis: time,
                        window: window.clone(),
                    };
                    state.touch_contacts.insert(id, contact.clone());
                    vec![(
                        RawTouchEvent {
                            id,
                            phase: RawTouchPhase::Down,
                            x: contact.x,
                            y: contact.y,
                            pointer_count: state.touch_contacts.len(),
                            pressure: None,
                            time_millis: time,
                        },
                        window,
                ...
                wl_touch::Event::Motion { time, id, x, y } => {
                    let pointer_count = state.touch_contacts.len();
                    let Some(contact) = state.touch_contacts.get_mut(&id) else {
                        return;
                    };
                    contact.x = x as f32;
                    contact.y = y as f32;
                    contact.time_millis = time;
                    vec![(
                        RawTouchEvent {
                            id,
                            phase: RawTouchPhase::Move,
                            x: contact.x,
                            y: contact.y,
                            pointer_count,
                            pressure: None,
                ...
                wl_touch::Event::Up { time, id, .. } => {
                    let pointer_count = state.touch_contacts.len();
                    let Some(contact) = state.touch_contacts.remove(&id) else {
                        return;
                    };
                    vec![(
                        RawTouchEvent {
                            id,
                            phase: RawTouchPhase::Up,
                            x: contact.x,
                            y: contact.y,
                            pointer_count,
                            pressure: None,
                    ...
                    state
                        ...
                        .map(|(id, contact)| {
                            (
                                RawTouchEvent {
                                    id,
                                    phase: RawTouchPhase::Cancel,
                                    x: contact.x,
                                    y: contact.y,
                                    pointer_count,
                                    pressure: None,
                                    time_millis: contact.time_millis,
                                },
                                contact.window,
                            )
                        })
                ...
                | wl_touch::Event::Orientation { .. } => Vec::new(),
                _ => Vec::new(),
            }
        };
        ...
        let mut dispatch = None;
        {
            ...
            match event {
                zwp_tablet_tool_v2::Event::ProximityIn { surface, .. } => {
                    ...
                    state.tablet_tools.entry(id).or_default().window = window;
                }
                zwp_tablet_tool_v2::Event::ProximityOut => {
                    let tablet = state.tablet_tools.entry(id).or_default();
                    if tablet.down {
                        tablet.pending_phase = Some(RawTouchPhase::Cancel);
                    } else {
                        tablet.window = None;
                    }
                }
                zwp_tablet_tool_v2::Event::Motion { x, y } => {
                    ...
                    tablet.x = x as f32;
                    tablet.y = y as f32;
                    tablet.pending_motion = true;
                }
                zwp_tablet_tool_v2::Event::Pressure { pressure } => {
                    state.tablet_tools.entry(id).or_default().pressure =
                        (pressure as f32 / 65535.0).clamp(0.0, 1.0);
                }
                zwp_tablet_tool_v2::Event::Down { .. } => {
                    let tablet = state.tablet_tools.entry(id).or_default();
                    tablet.down = true;
                    tablet.pending_phase = Some(RawTouchPhase::Down);
                }
                zwp_tablet_tool_v2::Event::Up => {
                    state.tablet_tools.entry(id).or_default().pending_phase =
                        Some(RawTouchPhase::Up);
                }
                zwp_tablet_tool_v2::Event::Frame { time } => {
                    let touch_count = state.touch_contacts.len();
                    let tablet_count = state
                        .tablet_tools
                        .values()
                        .filter(|tablet| tablet.down)
                        .count();
                    let tablet = state.tablet_tools.entry(id.clone()).or_default();
                    tablet.time_millis = time;
                    let phase = tablet.pending_phase.take().or_else(|| {
                        (tablet.down && tablet.pending_motion).then_some(RawTouchPhase::Move)
                    });
                    tablet.pending_motion = false;
                    if let (Some(phase), Some(window)) = (phase, tablet.window.clone()) {
                        dispatch = Some((
                            RawTouchEvent {
                                id: (tool.id().protocol_id() | 0x4000_0000) as i32,
                                phase,
                                x: tablet.x,
                                y: tablet.y,
                                pointer_count: (touch_count + tablet_count).min(32),
                                pressure: Some(tablet.pressure),
                                time_millis: time,
                            },
                            window,
                        ));
                        if matches!(phase, RawTouchPhase::Up | RawTouchPhase::Cancel) {
                            tablet.down = false;
                            tablet.pressure = 0.0;
                            if phase == RawTouchPhase::Cancel {
                                tablet.window = None;
                            }
                        }
                    }
